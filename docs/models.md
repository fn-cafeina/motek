...
type User struct {
    ...
    Password  string    `json:"-"` // nunca se retorna en JSON
...
type OrdenTrabajo struct {
    ...
    FechaEntrega   *time.Time `json:"fecha_entrega"` // nullable
...
type Factura struct {
    ...
    FechaVencimiento    *time.Time `json:"fecha_vencimiento"` // nullable
    Notas               string     `json:"notas"`
    CreadoEn            time.Time  `json:"creado_en"`
    ActualizadoEn       time.Time  `json:"actualizado_en"`
}
